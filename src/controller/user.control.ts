...
async function getList(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development' && users.length > 1) console.log('User list fetched');
  res.status(200).json(users);
}

async function getInfo(req: Request, res: Response): Promise<void> {
  const user = await userService.getInfo(req.user.id);
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: user info fetched`);
...
async function patchInfo(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: user info edited`);
  res.status(200).json(user);
}
...
async function patchPassword(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: user password changed`);
...
async function getProducts(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: products posted by the user`);
...
async function getArticles(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: articles posted by the user`);
...
async function getLikedProducts(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: favorite products`);
  res.status(200).json(products);
}
...
async function getLikedArticles(req: Request, res: Response): Promise<void> {
  ...
  if (NODE_ENV === 'development') console.log(`User${req.user.id}: favorite articles`);
  res.status(200).json(articles);
}
